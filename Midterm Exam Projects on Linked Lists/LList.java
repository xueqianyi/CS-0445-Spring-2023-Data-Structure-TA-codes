...
public class LList<T> implements ListInterface<T> {
    public Node firstNode; // Reference to first ndoe of chain
    ...
    public LList(LList<T> old) {
        Node tmp = null;
        for (Node node = old.firstNode; node != null; node = node.next) {
    ...
    public boolean equals(LList<T> rhs) {
        Node old = firstNode;
        for (Node node = rhs.firstNode; node != null && old != null; node = node.next, old = old.next) {
            if (node != old)
                return false;
            if ((node.next == null && old.next != null) || (node.next != null && old.next == null))
                return false;
        }
        return true;
    }
